...
(defvar +doom-dashboard-banner-file "banner.png"
...
(defvar +doom-dashboard-banner-dir user-emacs-directory
...
(defvar +doom-dashboard-pwd-policy 'last
...
;; (add-hook 'after-init-hook #'+doom-dashboard-init-h)
(add-hook 'emacs-startup-hook #'+doom-dashboard-init-h)
;;
;;; Faces
...
What it is set to is controlled by `+doom-dashboard-pwd-policy'."
  (if pwd
      (with-current-buffer (doom-fallback-buffer)
        ;; (message "Changed dashboard's PWD to %s" pwd)
...
If RETURN-P, return the message as a string instead of displaying it."
  (funcall (if t #'format #'message)
           "Emacs loaded %d packages in %s"
