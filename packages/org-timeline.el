...
This will be shown at the beginning of the block's line."
  (--if-let (org-entry-get (org-get-at-bol 'org-marker) "TIMELINE_CAT" t)
      it
    (let ((is-clocked (save-excursion (search-forward "Clocked:" (line-end-position) t))))
      (if (and is-clocked org-timeline-clocked-in-new-line)
          "$"
        nil))))

(defun org-timeline--get-no-overlap ()
  "Whether this block can overlap in timeline."
  (--if-let (org-entry-get (org-get-at-bol 'org-marker) "TIMELINE_NO_OVERLAP" t)
      it
    (let ((is-clocked (save-excursion (search-forward "Clocked:" (line-end-position) t))))
      (if (and (not is-clocked) org-timeline-overlap-in-new-line)
          t
        nil))))
        ...
        (when (member type (list "scheduled" "clock" "timestamp"))
          (let ((duration (org-get-at-bol 'duration))
            ...
            (let* ((hour (/ time-of-day 100))
                   ...
                   (cat (org-timeline--get-cat))
                   (no-overlap (org-timeline--get-no-overlap)))
...
(defun org-timeline--generate-timeline ()
  ...
  (let* ((start-offset (* org-timeline-start-hour 60))
    ...
    (cl-labels ((get-start-pos (current-line beg) (+ 1 5 (* (- current-line 1) (+ 5 (length slotline))) (/ (- beg start-offset) 10)))
